...
def generate_speech(reference_audio_path, reference_text, target_text, output_path, max_new_tokens=2000):
    ...
    logger.info(f"[CHECK] SNAC model: {getattr(snac_model, 'model_name', str(snac_model))}")
    logger.info(f"[CHECK] Tokenizer: {getattr(tokenizer, 'name_or_path', str(tokenizer))}")
    logger.info(f"[CHECK] Model device: {getattr(model, 'device', 'unknown')}, dtype: {getattr(model, 'dtype', 'unknown')}")
    logger.info(f"[CHECK] Batch reload: model id {id(model)}, snac_model id {id(snac_model)}")
    ...
    # Alignment check: log similarity between reference text and target text
    sm = difflib.SequenceMatcher(None, reference_text, target_text)
    logger.info(f"[CHECK] Reference/Target text similarity: {sm.ratio():.3f} (1.0=max, 0.0=min)")
    ...
    logger.info(f"[CHECK] Prompt (input_ids) length: {len(input_ids)}")
    context_window = getattr(model.config, 'n_positions', getattr(model.config, 'max_position_embeddings', 4096))
    logger.info(f"[CHECK] Model context window: {context_window}")
    if len(input_ids) > context_window:
        logger.warning(f"[WARNING] Prompt length ({len(input_ids)}) exceeds model context window ({context_window})! Prompt will be truncated.")
    try:
    ...
    logger.info(f"[CHECK] input_tensor device: {input_tensor.device}, dtype: {input_tensor.dtype}")
